...
<html <?php language_attributes(); ?>>
  <head>
	...
	<?php 
		...
		$maps_height = $get_meta['print_content_maps_width'];		
	if (!empty($favicon) ){ ?>
	...
	if ( !empty(get_post_meta( get_the_ID(), 'gp_map', true ))) :
		...
		if ( !empty(get_post_meta( $marker_map->ID, 'gp_project', true ))) :
			...
			if ( empty ( $display_map ) ) : 
				$link = get_the_permalink(); $map = str_replace('markers', 'markers/export', $link);	?>		
			<iframe id="gpmaps¨_print" src="<?php echo $map; ?>" width="100%" height="<?php echo $maps_height; ?>"></iframe>
			<?php endif; ?>
