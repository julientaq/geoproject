...
<html <?php language_attributes(); ?>>
  <head>
	...
	<?php 
		...
		$display_cape = $get_meta['print_content_capes'];		
		$capes_height = $get_meta['print_content_capes_width'];		
	if (!empty($favicon) ){ ?>
...
$print = $get_meta['print_js'];
	...
	if (empty($def) ) : 
		...
		@media print {
			...
			.entry-content blockquote::before, .cape-content blockquote::before, .marker-content blockquote::before{top:20px!important;}
		<?php if (!empty ($print) ) : ?>
...
<?php 
	...
	<div class="row set-icons">
		<div class="col-lg-12">
			...
			<?php 
			if ( empty ( $display_cape ) ) : 
				$link = get_the_permalink(); 
				$cape = str_replace('capes', 'capes/home', $link);	?>		
			<iframe id="gpcapes_print" src="<?php echo $cape; ?>" width="100%" height="<?php echo $capes_height; ?>"></iframe>
			<?php endif; ?>
